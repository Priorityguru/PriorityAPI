...
function setCookie(name_0, value_0, expires, path){
  if (uriEncoding) {
  ...
  setCookieImpl(name_0, value_0, toDouble(!expires?{l:0, m:0, h:0}:fromDouble(expires.jsdate.getTime())), '', path, false);
}
...
function getHyperLink(obj){
  ...
  if (!!h && h.jsArray.length > 0) {
    ...
    'HyperLinks' in jo.jsObject && (hyperlinks = dynamicCast($get_0(jo, 'HyperLinks'), 7));
  }
  if (hyperlinks) {
    return hyperlinks.jsArray;
  }
  return null;
}
...
function $SetUsernamePassword(this$static, app, username, password, activeDirectory, authAudience){
  this$static.Username = username + '\t' + app;
  $clinit_PriCommon();
  authAudience == null || authAudience.length == 0 || (this$static.Username += '\t' + authAudience);
...
function GetWCFServiceClient(conHolder){
  ...
  $SetUsernamePassword(con, App, Login, Pwd, ActiveDirectory, AuthAudience);
...
function SetWCFServiceClientParams(login, pwd, accessToken, computerName, windowsUser, authAudience){
  $clinit_PriCommon();
...
var ActiveDirectory = 0, App, AppId, AppKey, AppKeyFunction = null, AppName, AuthAudience = null, ComputerName, DName, DebugServer = 0, GPSLocation, Host, Lang = 0, Login, Pwd, WCFVersion = null, WindowsUser;
...
function ParseJO(jo, retObj, that, operation){
  ...
  if (jo.Form[0].ClearLine && jo.Form[0].ClearLine.length > 0) {
    for (i = 0; i < jo.Form[0].ClearLine.length; i++) {
      ...
      global_forms_rows[table] && (global_forms_rows[table][line] = {});
      var gform = that.GetJsTableObject(table);
  ...
  (jo.Form[0].fatalerror == 1 || jo.Form[0].fatalerror == 2) && (retObj.fatalerror = 1);
...
function $LoginB(this$static, username, pwd, accessToken, computerName, windowsUserName, authAudience, onSuccess, onError){
  var con;
  SetWCFServiceClientParams(username, pwd, accessToken, computerName, windowsUserName, authAudience);
...
_.Login = function Login_0(configuration, onSuccess, onError){
  ...
  $LoginB(this, conf.username, conf.password, conf.accessToken, conf.devicename, null, conf.authAudience, onSuccess, onError);
}
;
_.LoginB = function LoginB(username, pwd, accessToken, computerName, windowsUserName, authAudience, onSuccess, onError){
  $LoginB(this, username, pwd, accessToken, computerName, windowsUserName, authAudience, onSuccess, onError);
}
;
...
_.OnPriResponse = function OnPriResponse_31(response){
  ...
  try {
    ...
    if (jval) {
      ja = jval.isArray_0();
      !!ja && ja.jsArray.length > 0 && (message = getHyperLinkedText($get(ja, 0).isObject(), 'message'));
    }
    loginResult = ret;
    if (ret == 1) {
      $clinit_PriCommon();
      ...
      HtmlSvcCookie = $getConstantString('htmlsvccookie', '');
      HasUserValuesFeature = ($clinit_Boolean() , $getConstantInt('hasuservalues', 0) == 1?TRUE_0:FALSE_0);
      MGCookie != null && MGCookie.length > 0 && setCookie('PriUser', MGCookie, tenYearsFromNow, '/');
      DName != null && setCookie('Environment', DName, tenYearsFromNow, '/');
      if (HtmlSvcCookie != null && HtmlSvcCookie.length > 0) {
        setCookie('PriUserV2', HtmlSvcCookie, tenYearsFromNow, '/');
        setCookie('PriUserV2', HtmlSvcCookie, tenYearsFromNow, '/html/priority/primail/' + App);
        setCookie('PriUserV2', HtmlSvcCookie, tenYearsFromNow, '/html/priority/netfiles/' + App);
        setCookie('PriUserV2', HtmlSvcCookie, tenYearsFromNow, '/html/priority/apps/' + App);
      }
...
function $Input(this$static, proc, onSuccess){
  var chooseField, chooseFields, columnName, columntype, editField, editFields, field, formName, helpString, i, isboolean, ispassword, mandatory, name_0, op, opId, operator, operators, par, parameters, readonly, ret, selected, simplechoose, text_0, title_0, title0, value_0, value1, width_0, zoom, zoomtype, op_0;
  ...
  if (!!parameters && parameters.jsArray.length > 0) {
    ...
    if (editFields) {
      for (i = 0; i < editFields.jsArray.length; i++) {
        ...
        formName = getJSONStringProperty(editField, 'tname');
        columnName = getJSONStringProperty(editField, 'cname');
        $InputFieldAdd(this$static, ret, field, helpString, isboolean, ispassword, mandatory, operator, simplechoose, readonly, title0, columntype, value_0, value1, width_0, zoomtype, zoom, formName, columnName);
      }
    }
...
function $InputFieldAdd(this$static, retObj, fieldId, helpstring, isboolean, ispassword, mandatory, operator, simplechoose, readonly, title_0, columntype, value_0, value1, width_0, zoomtype, zoom, formName, columnName){
  ...
  $InputFieldAddNative(retObj, fieldId, helpstring, isboolean, ispassword, mandatory, operator, simplechoose, readonly, title_0, value_0, value1, width_0, zoomtype.name_0 != null?zoomtype.name_0:'' + zoomtype.ordinal, format, typestr, codestr, zoom, formName, columnName);
}

function $InputFieldAddNative(retObj, fieldId, helpstring, isboolean, ispassword, mandatory, operator, simplechoose, readonly, title_0, value_0, value1, width_0, zoomtype, format, typestr, codestr, zoom, formName, columnName){
  var field = {};
  field.field = fieldId;
  field.helpstring = helpstring;
...
function $ProcMessage(this$static, curmsg, onSuccess){
  ...
  HyperLinks = getHyperLink(curmsg);
  InvokeMessage(this$static, msgtext, mt.value_0, HyperLinks, onSuccess);
}
...
function $ProcStep(this$static, jo, operation, newStep, onSuccess, onError){
  var curmsg, exmail, messages, msg, msgs, n, pa, proc, s, step, stepobj, ret, ret_0, urls, ret_1, data_0, jurls, jurl, url_0;
  ...
  switch (step) {
    ...
    case 'upload':
      ret_1 = {};
      ret_1.proc = this$static.GetProc();
      ret_1.type = 'upload';
      onSuccess(ret_1);
      break;
    case 'download':
...
_.DocumentOptions = function DocumentOptions(ok, selectedFormat, options, onSuccess, onError){
  ...
  if (ok == 0) {
    ...
    if (optionsMode == 2) {
      opt = options;
      pdf = opt.pdf == 1;
      (opt.mode == 'signature' || opt.signature == 1) && ($clinit_PriCommon() , 6 <= BuildVersion)?(mode = 8):opt.mode == 'automail'?(mode = 5):opt.mode == 'signature_preview' && (mode = 7);
      opt.word == 1 && (mode += 10);
    }
     else {
      pdf = optionsMode == 1;
...
function InvokeMessage(thisProc, message, messageType, HyperLinks, onSuccess){
  var ret = {};
  ret.proc = thisProc.GetProc();
  ret.type = 'message';
  ret.message = message;
  ret.hyperlinks = HyperLinks;
